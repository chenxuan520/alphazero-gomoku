...
## 8. 配方切换：600-sim 饱和 → 900-sim 主线（2026-08-25）

**动因**：600-sim 续训 iter30/35 外部持平（56.4/55.8@48），内部 gate 停涨。
用户决策：配方 B（900 sims）扶正为主线。

**逐预算外测结论**（main900-iter2 = latest.30 + 300 步 900-sim 微调）：

| 预算 | latest.30 | main900-iter2 |
|---|---:|---:|
| 48 sims | 56.4% (n=360) | 55.8% (n=120) |
| 96 sims | 53.5% (n=200) | **60.0% (n=100)** |
| 600 sims | 65.0% (n=40) | 62.5% (n=40) |

判读：iter2-best 0.775@900 的 gate 读数主要来自"预算扩大分辨率"，48-sim
交互棋力与 600-sim 配方平级；96 档 +6.5pp 是唯一方向性亮点（n=100，未显著）。
当前棋力冠军仍按 48/96/600 联合证据记为 **latest.30（28be769f）**。

**吞吐修正**（user report: CPU 没吃满）：长尾局使 40 局×42 workers 平均每轮仅
~14 核实际做功。自 2026-08-25 15:00 起主线改为 80 局 × 48 workers × 16 训练线程，
单轮时长不变但每轮样本 ~3840（翻倍），队列摊薄长尾；实测 4743% 持续打满。

**900-sim gate 口径**：`--gate-every 3`。链上读数：iter2 0.775↑(对真基线) →
iter3 0.550 → iter6 0.400（best 仍为 iter2）。
