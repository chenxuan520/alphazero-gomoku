...
| 2000 sims | 500ms | 20.8% | 24 |
| 512 sims + VCF | 1500ms | 50.0% / 50.0%（72+24 局两批：**合并 48/96**） | 96 |

**解读**：无战术层加持时，我们任意单步预算都打不过 Rapfi 的"舒适区"。要咬下一口，需要把模拟量顶到 512 并且 Rapfi 还得在亚秒预算内——而 2000 sims 实测与 512 落在同一误差带，**模拟量红利在 512 后已经饱和**。这就是"通用 AZ-lite"与"领域特化引擎"在五子棋上的真实代差。表内最后一行说明：这个代差在接入 VCF 战术完成层后被闭合到五五开，详见 §11。
...
## 13. 附录: Rapfi 模仿种子实验（2026-09-15，已证伪并停止）

路线 B1（Rapfi 内战数据做 one-hot 监督微调，再上 gate）已按可复现口径跑完，结论为**无效，停止**：

- 语料：488 局 Rapfi 内战 → 19,871 条样本（`tools/rapfi_sl_seed.py`，纯 pipeline 化）。
- 试点（5.9k 样本/750 步/lr 1e-4）：challenger vs best80 = 0.375；对外 Rapfi@150ms = 1/24（与 best80 基线持平）。
- 正式线（19.9k 样本/最多 40 轮 × 250 步/lr 2e-4）：gate 逐档 0.325 / 0.250 / 0.325 / 0.275——**离 best80 越来越远**；对外 Rapfi@500ms（96 sims）= **0/24**，比 best80 基线（2/24）还低一档。已手动停止。
- 解读：one-hot 模仿改变了先验方向，但 20k 小规模静态标签没有把价值头/搜索结构抬上去；“更像 Rapfi”不等于“对外更强”（在没带动不动我们要做的 VCT/TSS 那一步之前，教师战术不可能一针见血进搜索）。

**这条路线在本机可支预算下的最终裁决**：B1 不成立。剩下的可行真路径只剩两条：内部做 Rapfi 级别的 TSS/VCT（A，纯自研，长），或加机器跑更长自对弈网络扩展（本机通量此前已证伪）。
